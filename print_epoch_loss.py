...
def print_epoch_loss(phase, epoch_id, loss,psnr,ssim, time_epoch, new_val_loss):
  ...
  print('{}[Epoch: {}, \t {}] Loss: {:.6f}\t psnr: {:.6f}\t ssim: {:.6f}\t (Stopwatch: {}){}'.format(
    UP_CLEAR_AND_RETURN,
    epoch_id,
    phase,
    loss,
  ...
  msg = '[Epoch: {}, \t {}] Loss: {:.6f}\t psnr: {:.6f}\t ssim: {:.6f}\t (Stopwatch: {}){}'.format(
        epoch_id, phase, loss, psnr, ssim, stopwatch(time.time(), time_epoch), message_new_val_loss
    )
  
  if phase == 'Tst':
    logging.getLogger("train").info(msg+"\n")
  else:
      logging.getLogger("train").info(msg)
